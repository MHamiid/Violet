...
namespace Violet {
	...
	struct Renderer2DData {
		const uint32_t MaxObjectsPerDraw = 10000;   //Batch 10000 object togther
	...
	*/
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const glm::vec4& color)
	{
		...
		*/
		//Bottom-Left
		AddVertexToBuffer({ position.x - (size.x / 2.0f), position.y - (size.y / 2.0f), position.z }, color,  glm::vec2(0.0f, 0.0f));
		//Bottom-Right
		AddVertexToBuffer({ position.x + (size.x / 2.0f), position.y - (size.y / 2.0f), position.z }, color, glm::vec2(1.0f, 0.0f));
		//Top-Right
		AddVertexToBuffer({ position.x + (size.x / 2.0f), position.y + (size.y / 2.0f), position.z }, color, glm::vec2(1.0f, 1.0f));
		//Top-Left
		AddVertexToBuffer({ position.x - (size.x / 2.0f), position.y + (size.y / 2.0f), position.z }, color, glm::vec2(0.0f, 1.0f));
	...
	void Renderer2D::AddVertexToBuffer(const glm::vec3& position, const glm::vec4& color, glm::vec2& textureCoordinates)
	{
